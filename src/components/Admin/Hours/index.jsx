...
const Hours = () => {
  ...
  const isLoadingCustomWorkingHours = useSelector((state) => state.isLoadingCustomWorkingHours);
  const isLoadingDisableDays = useSelector((state) => state.isLoadingDisableDays);
  ...
  const [deleteDisableDayModalOpen, setDeleteDisableDayModalOpen] = useState(false);
  ...
  const handleAddBlock = () => setNewBlocks([...newBlocks, { day: "", start: "", end: "" }]);
  const handleRemoveBlock = (index) => setNewBlocks(newBlocks.filter((_, i) => i !== index));
  ...
  const handleSubmit = async () => {
    ...
    if (!valid.length) return setError("Debes completar al menos un bloque válido.");
    ...
    response.success ? (toast.success(response.message), setModalOpen(false), dispatch(getWorkingHours(selectedWorker))) : toast.error(response.message);
  };

  const handleCreateCustomHour = async () => {
    if (!formData.dayOfWeek || !formData.startTime || !formData.endTime || formData.endTime <= formData.startTime) {
      return setError("Completá todos los campos con horas válidas.");
    }
    const response = await dispatch(createCustomWorkingHour({ ...formData, workerId: Number(selectedWorker) }));
    response.success ? (toast.success(response.message), setCustomModalOpen(false), dispatch(getCustomWorkingHours(selectedWorker))) : toast.error(response.message);
  };

  const handleEditSubmit = async () => {
    if (!editStart || !editEnd || editStart >= editEnd) return toast.error("Horas inválidas");
    const response = await dispatch(editWorkingHours({ startTime: editStart, endTime: editEnd }, hourToEdit.id));
    response.success ? (toast.success(response.message), setEditModalOpen(false), dispatch(getWorkingHours(selectedWorker))) : toast.error(response.message);
  };

  const handleCustomEditSubmit = async () => {
    if (!editCustomStart || !editCustomEnd || editCustomStart >= editCustomEnd) return toast.error("Horas inválidas");
    const response = await dispatch(editCustomWorkingHour({ startTime: editCustomStart, endTime: editCustomEnd }, customHourToEdit.id));
    response.success ? (toast.success(response.message), setEditCustomModalOpen(false), dispatch(getCustomWorkingHours(selectedWorker))) : toast.error(response.message);
  };
  ...
  const handleDisableDaySubmit = async () => {
    ...
    const response = await dispatch(createDisableDay({ ...disableDayData, workerId: Number(selectedWorker) }));
    response.success ? (toast.success(response.message), setDisableDayModalOpen(false), dispatch(getDisableDays(selectedWorker))) : toast.error(response.message);
  }

  const handleDeleteHour = async (id) => {
    const response = await dispatch(deleteWorkingHour(id));
    response.success ? (toast.success(response.message), setDeleteModalOpen(false), dispatch(getWorkingHours(selectedWorker))) : toast.error(response.message);
  };

  const handleDeleteCustomHour = async (id) => {
    const response = await dispatch(deleteCustomWorkingHour(id));
    response.success ? (toast.success(response.message), setDeleteCustomModalOpen(false), dispatch(getCustomWorkingHours(selectedWorker))) : toast.error(response.message);
  };

  const handleDeleteDisableDay = async (id) => {
    const response = await dispatch(deleteDisableDay(id));
    response.success ? (toast.success(response.message), setDeleteDisableDayModalOpen(false), dispatch(getDisableDays(selectedWorker))) : toast.error(response.message);
  }

  if (isLoadingWorkers) return <Loading loadingText="Cargando trabajadores..." />;
  ...
  return (
    <section className="max-w-7xl mx-auto p-4">
      ...
      {selectedWorker && !isLoadingWorkingHours && !isLoadingCustomWorkingHours && !isLoadingDisableDays && (
        <div>
          <div className="mt-4">
            ...
            />
            <button onClick={() => setModalOpen(true)} className="text-sm text-green-600 underline">
              Crear horarios
            </button>
          </div>
          ...
          <div className="mt-6">
            ...
            />
            <button onClick={() => setCustomModalOpen(true)} className="text-sm text-green-600 underline">
          ...
          <div className="mt-6">
              <p className="font-semibold mb-2">Dias deshabilitados:</p>
                ...
                />
              <button onClick={() => setDisableDayModalOpen(true)} className="text-sm text-green-600 underline">
              Crear horario personalizado
            </button>
          </div>
        </div>
